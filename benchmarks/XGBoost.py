...
if __name__ == "__main__":
    ...
    for k in range(len(loc)):
        ...
        rmse_list.append(np.sqrt((error ** 2).sum()/len(error)))
        mape_list.append(np.mean(np.abs(error)/real))
        corr_list.append(np.corrcoef(real, pred)[0][1])
    ...
    print('test|\MAE-mean\RMSE-mean\MAPE-mean\CORR-mean')
    log = '{:d}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}'
    print(log.format(1, mae_list.mean(), rmse_list.mean(), mape_list.mean(), corr_list.mean()))
